...
from teamspace.models import (
    User, Projects, Skills, UserSkills, Teams, TeamMembers, MatchScores, Evaluations, Portfolios, UserEmbedding, ProjectEmbedding
)
...
class Command(BaseCommand):
    ...
    def _create_users(self, cur, n_users: int) -> Tuple[List[int], dict]:
        ...
        self.stdout.write("Seeding UserEmbeddings…")
        users_for_embedding = User.objects.filter(user_id__in=inserted_ids)
        embeddings_to_create = []
        for user in users_for_embedding:
            embedding_text = f"{user.introduction} {user.specialty} {user.tech_stack} {user.major} {user.experience_level} {user.collaboration_style} {user.preferred_project_topics}"
            embedding_vector = generate_embedding(embedding_text)
            if embedding_vector:
                embeddings_to_create.append(
                    UserEmbedding(user=user, embedding=embedding_vector)
                )
        
        if embeddings_to_create:
            UserEmbedding.objects.bulk_create(embeddings_to_create)
            self.stdout.write(f"{len(embeddings_to_create)} user embeddings created.")
    ...
    def _create_projects(self, cur, n_projects: int, user_ids: List[int]):
        ...
        cur.execute("SELECT project_id FROM Projects ORDER BY project_id DESC LIMIT %s;", [len(rows)])
        project_ids = [pid for (pid,) in cur.fetchall()]

        self.stdout.write("Seeding ProjectEmbeddings…")
        projects_for_embedding = Projects.objects.filter(project_id__in=project_ids)
        embeddings_to_create = []
        for project in projects_for_embedding:
            embedding_text = f"{project.title} {project.description} {project.goal} {project.tech_stack}"
            embedding_vector = generate_embedding(embedding_text)
            if embedding_vector:
                embeddings_to_create.append(
                    ProjectEmbedding(project=project, embedding=embedding_vector)
                )
        
        if embeddings_to_create:
            ProjectEmbedding.objects.bulk_create(embeddings_to_create)
            self.stdout.write(f"{len(embeddings_to_create)} project embeddings created.")
